...
export class Shard {
    ...
    public async send(message: RawMessage, options: CrossHostMessage = {}) {
        ...
        return this.shard.evalOnManager(`this.netipc.send(JSON.parse('${JSON.stringify(message)}'), {resolve: false})`);
    }
    ...
    public async request(message: RawMessage, options: CrossHostMessage = {}) {
        ...
        return this.shard.evalOnManager(`this.netipc.request(JSON.parse('${JSON.stringify(message)}'))`);
    }
